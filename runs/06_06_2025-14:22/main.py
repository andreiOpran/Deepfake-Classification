...
def deepfake_classification_cnn_model():
    ...
    cnn_model.add(Conv2D(filters=512, kernel_size=(3, 3), activation='relu', padding='same')) # input_shape = (12, 12, 256)
...
def prepare_model_for_training(cnn_model):
    ...
    """
    cnn_model.compile(optimizer=Adam(learning_rate=0.00030), metrics=['accuracy'], loss='categorical_crossentropy')
    return cnn_model
