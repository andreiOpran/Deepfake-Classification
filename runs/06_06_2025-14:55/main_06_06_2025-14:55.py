...
def deepfake_classification_cnn_model():
    ...
    # resnet style conv -> batchnormalization -> relu
    # CONVOLUTIUONAL BLOCK 1
    # layer - relu removes negative values; input_shape 100x100 w/ RGB
    cnn_model.add(Conv2D(filters=64, kernel_size=(3, 3), padding='same'))  # input_shape = (100, 100, 3)
    ...
    cnn_model.add(Conv2D(filters=128, kernel_size=(3, 3), padding='same'))  # input_shape = (50, 50, 64)
    cnn_model.add(BatchNormalization())
    cnn_model.add(layers.Activation('relu'))
    cnn_model.add(Conv2D(filters=128, kernel_size=(3, 3), padding='same'))  # input_shape = (50, 50, 64)
    cnn_model.add(BatchNormalization())
    ...
    cnn_model.add(Conv2D(filters=256, kernel_size=(3, 3), padding='same'))  # input_shape = (25, 25, 128)
    cnn_model.add(BatchNormalization())
    cnn_model.add(layers.Activation('relu'))
    ...
    cnn_model.add(Conv2D(filters=512, kernel_size=(3, 3), padding='same'))  # input_shape = (12, 12, 256)
    cnn_model.add(BatchNormalization())
    cnn_model.add(layers.Activation('relu'))
    cnn_model.add(Conv2D(filters=512, kernel_size=(3, 3), padding='same'))
    cnn_model.add(BatchNormalization())
    cnn_model.add(layers.Activation('relu'))
    cnn_model.add(MaxPooling2D(pool_size=(2, 2)))
    cnn_model.add(Dropout(0.5))
    ...
    cnn_model.add(GlobalAveragePooling2D())  # for better generalization outputs 512 values instead of 36864
    cnn_model.add(Dense(units=256, activation='relu', kernel_regularizer=l2(0.01)))
    cnn_model.add(Dropout(0.5))
    cnn_model.add(Dense(units=128, activation='relu', kernel_regularizer=l2(0.01)))
